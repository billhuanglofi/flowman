...
func TestOpenProcessStateStore_returnsActionableError_whenOracleSupportDisabled(t *testing.T) {
	// Given
	config := oracle.Config{DSN: "example", User: "user", Password: "secret"}

	// When
	store, closeStore, err := oracle.OpenProcessStateStore(context.Background(), config)

	// Then
	if store != nil {
		t.Fatalf("expected no process state store without Oracle support, got %#v", store)
	}
	if closeStore != nil {
