...
import (
	...
	"github.com/billhuanglofi/flowman/internal/exporter/postman"
	"github.com/billhuanglofi/flowman/internal/model"
	"github.com/billhuanglofi/flowman/internal/storage"
)

func TestExportWorkspace_writes_postman_collection_and_reports_unsupported_flow_shapes(t *testing.T) {
	// Given
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	workspaceRoot := t.TempDir()
	writeExporterFixture(t, workspaceRoot)
	outputPath := filepath.Join(t.TempDir(), "payments.postman_collection.json")
	reportPath := filepath.Join(t.TempDir(), "report.json")

	// When
	result, err := postman.ExportWorkspace(ctx, postman.ExportOptions{WorkspacePath: workspaceRoot, OutputPath: outputPath, ReportPath: reportPath})

	// Then
	if err != nil {
		t.Fatalf("expected postman export to succeed: %v", err)
	}
	...
	if readErr != nil {
		t.Fatalf("expected exported collection to be readable: %v", readErr)
	}
	text := string(content)
	for _, want := range []string{"\"schema\": \"https://schema.getpostman.com/json/collection/v2.1.0/collection.json\"", "Create Payment", "Get Status", "{{FLOWMAN_PAYMENT_AUTH}}"} {
		if !strings.Contains(text, want) {
			t.Fatalf("expected exported collection to contain %q, got %s", want, text)
		}
	}
	if strings.Contains(text, "Bearer secret") {
		t.Fatalf("expected exported collection to avoid raw secrets, got %s", text)
	}
}
...
func TestExportWorkspace_strict_warnings_fail_before_output_write(t *testing.T) {
	...
	// When
	result, err := postman.ExportWorkspace(ctx, postman.ExportOptions{WorkspacePath: workspaceRoot, OutputPath: outputPath, ReportPath: reportPath, Strict: true})

	// Then
	if !errors.Is(err, postman.ErrStrictWarnings) {
	...
	if !strings.Contains(string(report), "postman-flow-multistep-unsupported") {
		t.Fatalf("expected warning report content, got %s", string(report))
	}
}

func writeExporterFixture(t *testing.T, root string) {
	t.Helper()
	...
	environment := model.Environment{Version: "v1", Name: "uat", BaseURL: "https://uat.api.example.test"}
	environment.Oracle = model.OracleDatabase{DSNEnv: "FLOWMAN_UAT_ORACLE_DSN", UserEnv: "FLOWMAN_UAT_ORACLE_USER", PasswordEnv: "FLOWMAN_UAT_ORACLE_PASSWORD"}
	environment.Trace = model.TraceConfig{TransactionID: model.TransactionIDExtraction{Header: "X-Transaction-ID"}, PollInterval: model.NewDuration(2 * time.Second), Timeout: model.NewDuration(30 * time.Second)}
